...
class Agent:
	...
	def remember(self, state, action, reward, next_state, terminal, next_valid_actions):
		self.memory.append((state, action, reward, next_state, terminal, next_valid_actions))

	def replay(self):
		batch = random.sample(self.memory, min(len(self.memory), self.batch_size))
		for state, action, reward, next_state, terminal, next_valid_actions in batch:
			q = reward
			if not terminal:
	...
	def save(self, path):
		...
		pickle.dump(attr, open(os.path.join(path, 'agent_attr.pickle'), 'wb'))
		self.model.save(path)

	def load(self, path):
		path = os.path.join(path, 'agent_attr.pickle')
		print(path)
...
class QModelKeras:
	...
	def save(self, path):
		...
		self.model.save_weights(os.path.join(path, 'weights.hdf5'))

		attr = dict()
		for a in self.attr2save:
			attr[a] = getattr(self, a)
		pickle.dump(attr, open(os.path.join(path, 'model_attr.pickle'), 'wb'))

	def load(self, path, learning_rate):
		json_str = open(os.path.join(path, 'model.json')).read()
		...
		attr = pickle.load(open(os.path.join(path, 'model_attr.pickle'), 'rb'))
		for a in attr:
...
class QModelMLP(QModelKeras):
	...
	def build_model(self, n_hidden, learning_rate, drop_rate, activation='relu'):
		...
		model.add(keras.layers.Reshape((self.state_shape[0]*self.state_shape[1], ), input_shape=self.state_shape))
...
class QModelConv(QModelKeras):
	...
	def build_model(self, filter_num, filter_size, dense_units, learning_rate,
					activation='relu', dilation=None, use_pool=None):
...
def load_model(path, learning_rate):
	s = open(os.path.join(path, 'QModel.txt'), 'r').read().strip()
	models = {
		'Conv':QModelConv,
		'DenseOnly':QModelMLP,
		'MLP':QModelMLP,
	...
	model = models[s](None, None)
	model.load(path, learning_rate)
	return model
